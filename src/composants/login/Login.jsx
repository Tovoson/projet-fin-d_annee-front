...
const Login = () => {
  ...
  const handleLogin = () => {
    let currentUser = allUsers.find((user) => user.nom === logNom && user.mdp === mdpLog);
    if (currentUser) {
      console.log(currentUser)
    }
  }
  const getAllUsers = () => {
    // eto asiana enpoint utilisateur retraretra
    ...
    <div className="login-page">
      <div className="login">
        ...
        <form >
          ...
          <div className="psd">
            <input type={visibilite ? "text" : "password"} className="psd_champ" required
              onChange={(e) => setMdpLog(e.target.value)} />
            <div onClick={handleClick}>
              {visibilite ? <Visibility /> : <VisibilityOff />}
            </div>
          </div>
      ...
      <div className="register">
        ...
        <form onSubmit={handleRegister}>
          ...
          <input type="text" className='textIn' placeholder='Numero matricule' required
            onChange={(e) => setId(e.target.value)} />
          <input type="text" className='textIn' placeholder='Nom et prenom' required
            onChange={(e) => setNom(e.target.value)} />
          <div className="psd">
            <input type={visibiliteReg ? "text" : "password"} className="psd_champ" required placeholder='Mot de passe'
              onChange={(e) => setMdp(e.target.value)} />
            <div onClick={handleClickReg}>
              {visibiliteReg ? <Visibility /> : <VisibilityOff />}
            </div>
          </div>
          <Button variant='contained' onClick={handleRegister} className={btn ? 'btn' : ''} >
